...
const setup = async () => {
  ...
  const IArb = await ethers.getContractFactory('DexHunter');
  arb = await IArb.attach(config.DexHunterContract);
  balances = {};
  for (let i = 0; i < config.baseAssets.length; i++) {
    const asset = config.baseAssets[i];
    const interface = await ethers.getContractFactory('WETH9');
    const assetToken = await interface.attach(asset.address);
    const balance = await assetToken.balanceOf(config.DexHunterContract);
...
const logResults = async () => {
  console.log(`############# LOGS #############`);
    ...
    const interface = await ethers.getContractFactory('WETH9');
    const assetToken = await interface.attach(asset.address);
    balances[asset.address].balance = await assetToken.balanceOf(config.DexHunterContract);
