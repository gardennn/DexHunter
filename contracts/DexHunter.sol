...
/// @title Dex Arbitrage Hunter
/// @notice This is an experimental contract. Not intended for profit-making use.
...
contract DexHunter is Ownable {
  ...
  event StablesAdded(address[] stables);
  event SwapExecuted(address router, address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut);

  function addRouters(address[] calldata _routers) external onlyOwner {
    for (uint i=0; i<_routers.length; i++) {
      routers.push(_routers[i]);
    }
    emit RoutersAdded(_routers);
  }
  ...
  function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) private {
    ...
    emit SwapExecuted(router, _tokenIn, _tokenOut, _amount, tokenOutBalance);
  }
  ...
  /// @notice Base Asset > Altcoin > Base Asset
  ...
  /// @notice Base Asset > Altcoin1 > Altcoin2 > Base Asset
  ...
  function triDexTrade(address _router1, address _router2, address _router3, address _token1, address _token2, address _token3, uint256 _amount) external onlyOwner {
    ...
    swap(_router1,_token1, _token2,_amount);
    uint token2Balance = IERC20(_token2).balanceOf(address(this));
    uint token2TradeableAmount = token2Balance - token2InitialBalance;
    swap(_router2,_token2, _token3, token2TradeableAmount);
    uint token3Balance = IERC20(_token3).balanceOf(address(this));
    uint token3TradeableAmount = token3Balance - token3InitialBalance;
    swap(_router3,_token3, _token1, token3TradeableAmount);
    uint endBalance = IERC20(_token1).balanceOf(address(this));
    require(endBalance > startBalance, "Trade Reverted, No Profit Made");
  }


  /// @notice Base Asset > Altcoin > Stablecoin > Altcoin > Base Asset
